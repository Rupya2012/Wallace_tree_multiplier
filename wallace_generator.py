...
def write_testbench(
    ...
    row_count = len(result.rows)
    zero_a = "{{" + str(width) + "{1'b0}}, a}"
    zero_b = "{{" + str(width) + "{1'b0}}, b}"
    ...
    for ridx in range(row_count):
        lines.append(f"wire [{2 * width - 1}:0] row{ridx};")

    got_width = 2 * width + max(1, row_count.bit_length())
    all_ones = "{" + str(width) + "{1'b1}}"
    lines.extend(
        [
            f"wire [{got_width - 1}:0] got;",
            f"reg [{2 * width - 1}:0] want;",
            "integer i;",
            "",
        ]
    )

    port_map = ", ".join([".a(a)", ".b(b)"] + [f".row{ridx}(row{ridx})" for ridx in range(row_count)])
    lines.append(f"{module_name} dut ({port_map});")

    sum_terms = " + ".join(f"{{{got_width - (2 * width)}'b0, row{ridx}}}" for ridx in range(row_count))
    lines.append(f"assign got = {sum_terms};")
    lines.extend(
        [
            "",
            "task check;",
            "begin",
            f"    want = {zero_a} * {zero_b};",
            "    #1;",
            f"    if (got !== {{{got_width - (2 * width)}'b0, want}}) begin",
            f"        $display(\"FAIL a=%h b=%h got=%h want=%h\", a, b, got, {{{got_width - (2 * width)}'b0, want}});",
            "        $fatal;",
            "    end",
            "end",
            "endtask",
            "",
            "initial begin",
            f"    a = {width}'h0; b = {width}'h0; check();",
            f"    a = {width}'h1; b = {width}'h1; check();",
            f"    a = {all_ones}; b = {width}'h1; check();",
            f"    a = {width}'h1; b = {all_ones}; check();",
            f"    a = {all_ones}; b = {all_ones}; check();",
...
def generate_all(width: int = WIDTH, random_tests: int = 200) -> ReductionResult:
    ...
    write_testbench(result, f"tb_wallace{width}_n3_5.v", width=width, variant="optimized")
    return result
...
def generate_normal_all(width: int = WIDTH, random_tests: int = 200) -> ReductionResult:
    ...
    write_testbench(result, f"tb_wallace{width}_normal.v", width=width, variant="normal")
    return result
