...
def write_report(result: ReductionResult, path: str | Path, width: int = WIDTH) -> None:
    ...
    with path.open("w", encoding="utf-8") as f:
        f.write(f"Reduction report: standard_wallace_32_22, {width}x{width}\n")
...
def write_testbench(result: ReductionResult, path: str | Path, width: int = WIDTH) -> None:
    path = Path(path)
    module_name = _module_name(width)
    ...
    lines = [
        ...
        f"reg [{width - 1}:0] a;",
        f"reg [{width - 1}:0] b;",
        f"wire [{2 * width - 1}:0] row0;",
        f"wire [{2 * width - 1}:0] row1;",
        f"wire [{2 * width}:0] got;",
        f"reg [{2 * width - 1}:0] want;",
        "integer i;",
        "",
        f"{module_name} dut (.a(a), .b(b), .row0(row0), .row1(row1));",
        "assign got = {1'b0, row0} + {1'b0, row1};",
        "",
        "task check;",
        "begin",
        f"    want = {zero_a} * {zero_b};",
        "    #1;",
        "    if (got !== {1'b0, want}) begin",
        '        $display("FAIL a=%h b=%h got=%h want=%h", a, b, got, {1\'b0, want});',
        "        $fatal;",
        "    end",
        "end",
        "endtask",
...
def generate_all(width: int = WIDTH, random_tests: int = 200) -> ReductionResult:
    ...
    write_testbench(result, f"tb_wallace{width}_standard.v", width=width)
    return result
